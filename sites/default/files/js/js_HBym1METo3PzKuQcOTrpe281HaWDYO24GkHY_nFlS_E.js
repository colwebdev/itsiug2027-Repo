...
/* @license GPL-2.0-or-later https://www.drupal.org/licensing/faq */
(function(Drupal){Drupal.tagify=Drupal.tagify||{};Drupal.tagify.countSelectedTags=(identifier)=>{const tagsElement=identifier?document.querySelector(`.${identifier}`):null;return tagsElement?tagsElement.querySelectorAll('.tagify__tag').length:0;};Drupal.tagify.isTagLimitReached=(identifier,cardinality)=>cardinality>0&&Drupal.tagify.countSelectedTags(identifier)>=cardinality;Drupal.tagify.createLoadingTextMarkup=(identifier)=>{const tagsElement=document.querySelector(`.${identifier}`);if(!tagsElement)return;const loadingText=document.createElement('div');loadingText.className='tagify--loading-text hidden';loadingText.textContent=Drupal.t('Loading...');tagsElement.appendChild(loadingText);};Drupal.tagify.removeLoadingTextMarkup=(identifier)=>{const tagsElement=document.querySelector(`.${identifier}`);if(tagsElement){const loadingText=tagsElement.querySelector('.tagify--loading-text');if(loadingText)loadingText.remove();}};Drupal.tagify.highlightMatchingLetters=(inputTerm,searchTerm)=>{inputTerm=inputTerm==null?'':inputTerm.toString();searchTerm=searchTerm==null?'':searchTerm.toString();const escapedSearchTerm=searchTerm.replace(/[.*+?^${}()|[\]\\]/g,'\\$&');if(!escapedSearchTerm)return Drupal.checkPlain(inputTerm);const regex=new RegExp(`(${escapedSearchTerm})`,'gi');return inputTerm.split(regex).map((segment,index)=>index%2===1?`<strong>${Drupal.checkPlain(segment)}</strong>`:Drupal.checkPlain(segment)).join('');};})(Drupal);;
