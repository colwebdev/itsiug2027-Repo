...
class DxprBuilderFormatter extends FormatterBase {
  ...
  public function attachAssets($container_name, array &$element, $content, $html_format, $enable_editor, $mode, $dxpr_lang): array {
    ...
    if ($settings['mediaBrowser'] != '') {
      ...
      // Add urls to either "Entity Browser" or "Media Library" media browser.
    ...
    $aiEnabled = $dxprConfig->get('ai_enabled');

    // If ai_enabled is unset (NULL), default to enabled (TRUE)
    if ($aiEnabled === NULL) {
      $aiEnabled = TRUE;
    }
    ...
    $modelMap = [
      'dxai_kavya_m1' => 'kavya-m1',
      'dxai_kavya_m1_eu' => 'kavya-m1-eu',
    ];

    $aiModel = $dxprConfig->get('ai_model') ?: 'dxai_kavya_m1';
    $aiProviders = $dxprConfig->get('ai_providers') ?: [];

    $aiModel = $modelMap[$aiModel] ?? 'kavya-m1';

    $settings['aiAgent'] = [
      'enabled' => (bool) $aiEnabled,
      'model' => $aiModel,
      'engine' => 'dxai',
    ];

    // Only include providers parameter when the model is kavya-m1.
    if ($aiModel === 'kavya-m1' && !empty($aiProviders)) {
