...
class DxprBuilderService implements DxprBuilderServiceInterface {
   ...
   *   The logger factory.
   * @param \Drupal\dxpr_builder\Service\DxprBuilderKeyService $key_service
   *   The DXPR Builder key service.
   */
  public function __construct(
    ...
    DxprBuilderKeyService $key_service,
  ) {
  ...
  public function editorAttach(array &$element, array &$settings): void {
    ...
    if ($config->get('editor_assets_source') == 0) {
      $api_key = $this->keyService->getApiKey();
      if (empty($api_key)) {
      ...
      $settings['dxprAssetsParams'] = "jwt=$api_key";
    }
    else {
    ...
    // Ensure 'optimize' style is at the top if it exists.
    if (isset($styles['optimize'])) {
      $styles_list = ['optimize' => $styles['optimize']->label()] + $styles_list;
      unset($styles['optimize']);
    }
   ...
   * {@inheritdoc}
   */
  public function loadCmsElement(array $element_info, string $settings, array $data = [], ?AttachedAssets $assets = NULL): bool|string {
   ...
   * {@inheritdoc}
   */
  public function synchronizeImageStylesWithAcquiaDam(): void {
    $acquia_dam_settings = $this->configFactory->getEditable('acquia_dam.settings');
    if ($acquia_dam_settings->isNew()) {
      return;
    }

    $styles = array_keys($this->entityTypeManager->getStorage('image_style')
      ->loadMultiple());

    $acquia_dam_settings->set('allowed_image_styles', array_combine($styles, $styles))
      ->save();
  }
        ...
        foreach (Views::getEnabledViews() as $view) {
          try {
            ...
            foreach ($executable_view->displayHandlers as $id => $display) {
              ...
              // Build a map of relationship IDs to their labels.
              $relationships = [];
              foreach ($executable_view->getHandlers('relationship') as $id => $handler) {
                if (is_object($handler) && method_exists($handler, 'adminLabel')) {
                  $relationships[$id] = Html::escape($handler->adminLabel());
                }
              }
              ...
              foreach ($fields as $k => $field) {
                ...
                if (isset($field['relationship']) && !empty($field['relationship'])) {
                  $relationship_key = (string) $field['relationship'];
                  if (isset($relationships[$relationship_key]) && !empty($relationships[$relationship_key])) {
                    $field_name = '(' . $relationships[$relationship_key] . ') ' . $field_name;
                  }
                }
                $fields[$k] = $field_name;
              }
  ...
  private function parseForContent(array &$response, bool $enable_editor): void {
    ...
    // Remove root az-container from DOM before serialization.
    $this->stripRootContainer($doc);
  ...
  private function parseDocumentForTemplateLibrary(\DOMDocument $doc, array &$response): void {
    ...
    foreach ($nodes as $node) {
      if ($node instanceof \DOMElement) {
        ...
        // Font Awesome 4/5/6 backwards compatibility.
        foreach ($keys as &$key) {
          if (in_array($key, ['font_awesome_5_pro', 'font_awesome_5_free', 'font_awesome'])) {
            $key = 'font_awesome_6_free';
            $response['library'][] = 'dxpr_builder/elements.font_awesome_v4_shims';
          }
  ...
  private function parseDocumentForCleanup(\DOMDocument $doc) {
    ...
    $auto_gen_passthrough_attrs = [
      // DXPR_PASSTHROUGH_GEN_START.
      // Collapse panel.
      'collapsed',
      // Alert.
      'type',
      ...
      'grayscale',
      'fade_edges',
      // DXPR_PASSTHROUGH_GEN_END.
    ];

    $auto_gen_static_passthrough_attrs = [
      // DXPR_STATIC_PASSTHROUGH_GEN_START.
      // Number counter.
      'start',
      'end',
      'speed',
      ...
      'prefix',
      'postfix',
      // DXPR_STATIC_PASSTHROUGH_GEN_END.
    ];
    ...
    $dxprDataAttrsElements = $xpath->query('//*[contains(@class,"az-element") or contains(@class,"az-ctnr") or @data-dxpr-builder-libraries]');
    ...
    foreach ($dxprDataAttrsElements as $element) {
      ...
      $elDynamic = (strpos($el_name, 'dyn_') === 0);
      ...
      foreach ($data_attrs as $attr) {
        if ($element->hasAttribute($attr)) {
          ...
          // data-azb, data-azcnt.
          $clean_attr = str_replace('data-az', '', $clean_attr);
          $attrDynamic = (strpos($clean_attr, 'pt_') === 0);

          // Check if attribute is in static passthrough list first.
          if (in_array($clean_attr, $auto_gen_static_passthrough_attrs)) {
            continue;
          }

          // Otherwise check dynamic conditions.
          if ((!($el_name) or in_array($el_name, $dynamic_els) or $el_anim or $elDynamic)
            && (in_array($clean_attr, $passthrough_attrs) or $attrDynamic)) {
            continue;
          }
  ...
  /**
   * Removes the root az-container from the DOM if present.
   *
   * @param \DOMDocument $doc
   *   The DOMDocument to process.
   */
  private function stripRootContainer(\DOMDocument $doc): void {
    $xpath = new \DOMXpath($doc);
    $container = $xpath->query('//div[contains(@class,"az-container") and @data-azb="az_container"]')->item(0);
