...
(function (Drupal, drupalSettings, Sortable) {
  ...
  // Matches a URL ending in a supported image file extension. Built once
  // instead of on every validImgSrc() call.
  const IMG_SRC_PATTERN = new RegExp(
    '^(https?:\\/\\/)?' +
  ...
  Drupal.behaviors.tagifyAutocomplete = {
    attach: function attach(context) {
      ...
      elements.forEach((input) => {
        ...
        // Tag counting, limit checks and the loading-text markup live in the
        // shared Drupal.tagify helpers (js/tagify.helpers.js). Bind the limit
        // check to this widget's identifier/cardinality for readability.
        ...
        function validImgSrc(infoLabel) {
          return IMG_SRC_PATTERN.test(infoLabel);
        }
        ...
        function infoLabelMarkup(infoLabel) {
          ...
          // Info label markup (image or plain text). validImgSrc() result is
          // reused for the inner markup instead of being computed twice.
          const isImgSrc = validImgSrc(infoLabel);
          const isImage =
            isImgSrc || containsSVG(infoLabel) || containsImgTag(infoLabel);

          if (isImage) {
            const inner = isImgSrc
              ? `<img onerror="this.style.visibility='hidden'" src="${infoLabel}">`
              : infoLabel;
            return `<div id='tagify__tag__info-label-wrap' class='tagify__tag__info-label-wrap'><div class='tagify__tag-info-label-image'>${inner}</div></div>`;
          }

          return `<div id='tagify__tag__info-label-wrap' class='tagify__tag__info-label-wrap'><span class='tagify__tag-info-label'>${infoLabel}</span></div>`;
        }
        ...
        function handleAutocomplete(value, selectedEntities) {
          ...
          fetch(
            `${input.dataset.autocompleteUrl}${operator}q=${encodeURIComponent(value)}&selected=${selectedEntities}`,
  ...
  Drupal.behaviors.tagifySelect = {
    attach: function attach(context) {
      ...
      selectElements.forEach((select) => {
        ...
        // Tag counting and the limit check come from the shared Drupal.tagify
        // helpers (js/tagify.helpers.js).
        const isTagLimitReached = () =>
          Drupal.tagify.isTagLimitReached(identifier, cardinality);
        ...
        if (select.hasAttribute('disabled')) {
          input.setAttribute('disabled', select.getAttribute('disabled'));
        }
