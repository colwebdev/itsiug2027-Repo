...
class EntityAutocompleteTagifyUserList extends Textfield {
   ...
   *   The form element.
   */
  public static function processEntityAutocompleteTagifyUserList(array &$element, FormStateInterface $form_state, array &$complete_form): array {
  ...
  public static function valueCallback(&$element, $input, FormStateInterface $form_state): string|false|null {
    ...
    if (FALSE !== $input && is_array($input)) {
      $entity_ids = array_map(static fn(array $item) => $item['target_id'], $input);
   ...
   *     The user image.
   */
  public static function getTagifyDefaultValue(array $entities, string $info_label_template, string $image, string $image_style): string|false {
    $default_value = [];
    ...
    foreach ($entities as $entity) {
      ...
      $default_value[] = [
        ...
        'avatar' => \Drupal::service('tagify_user_list.user_image')->getImagePath($entity, $image, $image_style) ?: '/' . $tagify_user_list_path . '/images/no-user.svg',
        'editable' => FALSE,
      ];
    }
