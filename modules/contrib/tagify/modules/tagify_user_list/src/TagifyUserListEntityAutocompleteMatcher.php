...
class TagifyUserListEntityAutocompleteMatcher extends TagifyEntityAutocompleteMatcher {
  ...
  /**
   * Sets the user image resolver.
   *
   * @param \Drupal\tagify_user_list\UserImageResolverInterface $user_image_resolver
   *   The user image resolver.
   */
  public function setUserImageResolver(UserImageResolverInterface $user_image_resolver): void {
    $this->userImageResolver = $user_image_resolver;
  }
    ...
    if ($handler !== FALSE) {
      ...
      foreach ($entity_labels as $values) {
        foreach ($values as $entity_id => $label) {
          ...
          if ($label !== NULL) {
            $matches[$entity_id] = $this->buildTagifyUserListItem($target_type, (string) $entity_id, $label, $info_label, $selection_settings['image'], $selection_settings['image_style']);
          }
        }
      }
      if ($match_limit > 0) {
  ...
  protected function buildTagifyUserListItem(string $target_type, string $entity_id, string $label, ?string $info_label, string $image, string $image_style): array {
    ...
    return [
      ...
      'avatar' => $this->userImageResolver->getImagePath($entity, $image, $image_style) ?: '/' . $tagify_user_list_path . '/images/no-user.svg',
      'editable' => FALSE,
    ];
  }
