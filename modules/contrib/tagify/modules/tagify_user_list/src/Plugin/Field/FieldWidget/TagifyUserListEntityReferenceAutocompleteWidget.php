...
use Drupal\tagify_user_list\UserImageResolverInterface;
...
#[FieldWidget(
  ...
  label: new TranslatableMarkup('Tagify User List'),
  description: new TranslatableMarkup('An autocomplete text field with tagify support for user entity.'),
  field_types: [
    'entity_reference',
  ],
...
class TagifyUserListEntityReferenceAutocompleteWidget extends TagifyEntityReferenceAutocompleteWidget {
  ...
  /**
   * The user image resolver.
   *
   * @var \Drupal\tagify_user_list\UserImageResolverInterface
   */
  protected UserImageResolverInterface $userImageResolver;
  ...
  public static function create(ContainerInterface $container, array $configuration, $plugin_id, $plugin_definition): static {
    ...
    $instance->userImageResolver = $container->get('tagify_user_list.user_image');
    return $instance;
  }
  ...
  public static function defaultSettings(): array {
    return [
      'match_operator' => MatchOperator::StartsWith->value,
      'match_limit' => 10,
  ...
  public function settingsForm(array $form, FormStateInterface $form_state): array {
    ...
    $element['image'] = [
      ...
      '#options' => $this->userImageResolver->getImageOptions($target_type, $allowed_bundles),
      '#default_value' => $this->getSetting('image'),
      '#description' => $this->t('Image field used for the avatar.'),
    ];
    // Build the image style options from storage so the widget stays
    // compatible across supported core versions (avoids the deprecated
    // image_style_options() and the 11.4-only ImageDerivativeUtilities).
    $image_style_options = [];
