...
class EntityAutocompleteTagify extends Textfield {
  ...
  public function getInfo(): array {
    ...
    $info['#match_operator'] = MatchOperator::Contains->value;
   ...
   *   The form element.
   */
  public static function processEntityAutocompleteTagify(array &$element, FormStateInterface $form_state, array &$complete_form): array {
    ...
    $theme_resolver = \Drupal::service('tagify.theme_resolver');
    if ($theme_resolver->isGinActive()) {
      $element['#attached']['library'][] = 'tagify/gin';
    }

    if ($theme_resolver->isClaroActive()) {
    ...
    $element['#attributes']['data-match-operator'] = MatchOperator::fromSettings($element['#match_operator'])->toDataAttribute();
   ...
   * {@inheritdoc}
   */
  public static function valueCallback(&$element, $input, FormStateInterface $form_state): string|false|null {
   ...
   */
  public static function getTagifyDefaultValue(array $entities, string $info_label_template): string|false {
    ...
    foreach ($entities as $entity) {
      ...
      // Add the parent name for hierarchical terms. getEntityTypeId() returns
      // the machine name; the previous getEntityType() compared an object to a
      // string and never matched, so this branch was dead. Attach the parent
      // name to the item (not the outer list) to keep the JSON an array.
      if ($entity_id && $entity->getEntityTypeId() === 'taxonomy_term') {
        $item['parent_name'] = \Drupal::service('tagify.hierarchical_term_manager')->getParentName((string) $entity_id, $entity->bundle());
      }
