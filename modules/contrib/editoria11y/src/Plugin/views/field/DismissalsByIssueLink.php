...
class DismissalsByIssueLink extends Standard {
  ...
  public function render(ResultRow $values): MarkupInterface|string|ViewsRenderPipelineMarkup|GeneratedLink {
    ...
    if (!empty($value)) {
      ...
      $strung = is_object($value) && method_exists($value, '__toString') ? $value->__toString() : (string) $value;
      $translated = (new TestNames())->coreNames();
      if (array_key_exists($strung, $translated)) {
      ...
      $url = Url::fromRoute('view.ed11y_action.page_1', [
        'result_key' => $strung,
      ]);
