...
trait DeepCloningTrait {
  ...
  protected function cloneReferencedEntities($entity) {
    ...
    foreach ($entity_fields as $field_key => $entity_field) {
      ...
      // referencedEntities(), check issue #3089724, also it uses revision id.
      // Only call cloneReferencedParagraphsEntities for paragraph fields.
      if ($entity_field->getSetting('target_type') && $entity_field->getSetting('target_type') == 'paragraph') {
      ...
      if ($entity_field->getName() != 'type' && isset($value[0]['target_id'])) {
        ...
        // the current target ids with the new entities.
        ...
        if (
          ...
          foreach ($referenced_entities as $delta => $entity_reference) {
            ...
            if (!in_array($entity_reference->getEntityTypeId(), $this->getAllowedTypes())) {
              $values = ['target_id' => $entity_reference->id()];
              // Include other properties from the cloned field, e.g. alt text.
              foreach ($value[$delta] as $key => $val) {
                if ($key != 'target_id') {
                  $values[$key] = $val;
                }
              }
              $new_referenced_target_ids[] = $values;
              continue;
            }
  ...
  protected function cloneReferencedParagraphsEntities($field_key, $entity_field, &$entity) {
    $value = $entity_field->getValue();
    $new_value = [];
    // Unsaved inline blocks using paragraph form modes and IEF - simple.
    ...
    // IEF - complex form mode structure and saved inline blocks.
    elseif (
      ...
      $default_fields = \Drupal::service('entity_field.manager')->getBaseFieldDefinitions('paragraph');
      foreach ($value as $key => $item) {
        ...
        $new_paragraph = $paragraph_storage->create([
          'type' => $paragraph->getType(),
          'parent_type' => $entity->getEntityTypeId(),
          'parent_field_name' => $field_key,
        ]);
        $new_paragraph->isNew();
        foreach ($fields as $field) {
          $field_name = $field->getName();
          // Copy non-default field values to new paragraph.
          if (!in_array($field_name, array_keys($default_fields))) {
            $field_value = $field->getValue();

            if (!empty($field_value)) {
              // Recursively clone referenced paragraphs.
              if (isset($field_value[0]['target_id'])
                && $field->getSetting('target_type') == 'paragraph') {
                $this->cloneReferencedParagraphsEntities($field_name, $field, $new_paragraph);
              }
              else {
                $new_paragraph->set($field_name, $field_value);
              }
            }
          }
        }
        $new_value[$key]['entity'] = $new_paragraph;
      }
    }
